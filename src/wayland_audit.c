...
//   1. Detects when libvkintox.so is loaded (la_objopen)
...
//
// Usage: LD_AUDIT=/path/to/libvkintox-audit.so <game>
//
// Performance: la_objopen returns 0 for unrelated libraries, so la_symbind64
// is only called for wayland/vkintox/wine-related objects. Overhead for
...
static uintptr_t vkintox_cookie = 0;

// Address of our wl_proxy_add_listener wrapper in libvkintox.so
...
la_objopen(struct link_map *map, Lmid_t lmid, uintptr_t *cookie)
{
    ...
    if (strstr(name, "libvkintox.so"))
    {
        vkintox_cookie = *cookie;
             ...
             unsigned int *flags, const char *symname)
{
    ...
    if (*refcook == vkintox_cookie || *defcook == vkintox_cookie)
        return sym->st_value;
